...
/// One `from → to` rename within a single folder (plain filenames, not paths). Produced by the frontend
/// batch-rename engine (CPE-700) and applied together by {@link rename_many} (CPE-701, epic CPE-699).
...
/// A per-item failure during a batch rename — reported rather than aborting the whole batch.
...
/// Result of a batch rename: the net pairs applied, any per-item errors, and the inverse pairs to undo
/// the batch (valid only when `errors` is empty). The undo can be fed straight back into `rename_many`.
#[derive(Debug, serde::Serialize)]
struct RenameManyResult {
    renamed: Vec<RenamePair>,
    errors: Vec<RenameError>,
    undo: Vec<RenamePair>,
}

/// A concrete filesystem step: rename `src` → `dst` (plain names within the folder).
...
/// Order rename pairs into concrete steps that never clobber a name a later step still needs, breaking
/// cycles/swaps (a→b, b→a) with temporary names. Pure — no filesystem — so the tricky ordering is
/// unit-testable. Assumes pairs are pre-validated (unique targets; no target collides with an
/// out-of-set sibling). No-op pairs (`from == to`) are dropped.
fn plan_renames(pairs: &[(String, String)]) -> Vec<RenameStep> {
    let mut pending: Vec<(String, String)> =
        pairs.iter().filter(|(f, t)| f != t).cloned().collect();
    // Names still on disk that a pending rename must move away before that name can be reused.
    let mut sources: std::collections::HashSet<String> =
        pending.iter().map(|(f, _)| f.clone()).collect();
    let mut steps = Vec::new();
    let mut tmp_counter = 0usize;

    while !pending.is_empty() {
        // Prefer any rename whose target is free (not still occupied by a pending source).
        if let Some(idx) = pending.iter().position(|(_, t)| !sources.contains(t)) {
            let (f, t) = pending.remove(idx);
            sources.remove(&f);
            steps.push(RenameStep { src: f, dst: t });
        } else {
            // Every remaining target is occupied → a cycle. Break it: move one source aside to a temp
            // name (freeing its old name), then let it flow to its real target once that frees up.
            let (f, t) = pending.remove(0);
            let tmp = format!(".cpe-rename-tmp-{tmp_counter}");
            tmp_counter += 1;
            ...
            sources.remove(&f);
            pending.push((tmp, t)); // the temp isn't a source anyone waits on
        }
    }
    steps
}

/// Apply a batch of `{ from, to }` renames within `dir` as one collision-safe operation (CPE-701, epic
/// CPE-699). Renames are ordered so none clobbers a sibling still to be moved, with temp names resolving
/// swaps/cycles. Pre-validates every target (name-only, no separator, no collision with an out-of-set
/// sibling) and refuses the whole batch on a validation problem — nothing is renamed. Per-item
/// filesystem errors during apply are reported, not fatal. Returns the inverse pairs for undo.
#[tauri::command]
fn rename_many(dir: String, pairs: Vec<RenamePair>) -> Result<RenameManyResult, String> {
    let base = Path::new(&dir);
    if !base.is_dir() {
        return Err(format!("Not a folder: {dir}"));
    }
    let sources: std::collections::HashSet<&str> = pairs.iter().map(|p| p.from.as_str()).collect();
    let mut seen_targets: std::collections::HashSet<&str> = std::collections::HashSet::new();
    for p in &pairs {
        valid_entry_name(&p.from)?;
        let to = p.to.trim();
        if to.is_empty() {
            return Err("A target name is empty".to_string());
        }
        valid_entry_name(to)?;
        if p.from == p.to {
            continue; // no-op
        }
        if !seen_targets.insert(p.to.as_str()) {
            return Err(format!("Two files would both be named \"{}\"", p.to));
        }
        // A target may already exist on disk only if it's itself being renamed away; else it's a clobber.
        if base.join(to).exists() && !sources.contains(p.to.as_str()) {
            return Err(format!("\"{}\" already exists", p.to));
        }
    }
    ...
    for step in plan_renames(&plan_pairs) {
        if let Err(e) = fs::rename(base.join(&step.src), base.join(&step.dst)) {
    ...
    let renamed: Vec<RenamePair> = pairs.into_iter().filter(|p| p.from != p.to).collect();
    let undo: Vec<RenamePair> = renamed
        .iter()
        .map(|p| RenamePair {
            from: p.to.clone(),
            to: p.from.clone(),
...
mod tests {
    ...
    /// Simulate a plan against a name→content map, asserting no step ever clobbers a live name, and
    /// return the final name→content map so a caller can check the net effect. Pure — proves the
    /// ordering logic without touching disk.
    fn simulate_plan(initial: &[(&str, &str)], pairs: &[(&str, &str)]) -> std::collections::HashMap<String, String> {
        let mut fsmap: std::collections::HashMap<String, String> =
            initial.iter().map(|(n, c)| (n.to_string(), c.to_string())).collect();
        let owned: Vec<(String, String)> =
            pairs.iter().map(|(f, t)| (f.to_string(), t.to_string())).collect();
        for step in plan_renames(&owned) {
            assert!(
                !fsmap.contains_key(&step.dst),
                "plan clobbers {} while doing {}->{}",
                step.dst,
                step.src,
                step.dst
            );
            let content = fsmap.remove(&step.src).expect("step src must exist");
            fsmap.insert(step.dst.clone(), content);
        }
        fsmap
    }

    #[test]
    fn plan_renames_orders_a_shift_chain_without_clobbering() {
        // a->b, b->c must run as b->c THEN a->b, never clobbering b.
        let out = simulate_plan(&[("a", "A"), ("b", "B")], &[("a", "b"), ("b", "c")]);
        assert_eq!(out.get("b").map(String::as_str), Some("A"));
        assert_eq!(out.get("c").map(String::as_str), Some("B"));
        assert!(!out.contains_key("a"));
    }

    #[test]
    fn plan_renames_resolves_a_swap_via_a_temp_name() {
        // a<->b swap needs a temp name; contents must end up exchanged.
        let out = simulate_plan(&[("a", "A"), ("b", "B")], &[("a", "b"), ("b", "a")]);
        assert_eq!(out.get("a").map(String::as_str), Some("B"));
        assert_eq!(out.get("b").map(String::as_str), Some("A"));
        assert_eq!(out.len(), 2, "no temp files should be left behind");
    }

    #[test]
    fn plan_renames_resolves_a_three_cycle() {
        ...
        assert_eq!(out.get("b").map(String::as_str), Some("A"));
        assert_eq!(out.get("c").map(String::as_str), Some("B"));
        assert_eq!(out.get("a").map(String::as_str), Some("C"));
        assert_eq!(out.len(), 3);
    }

    #[test]
    fn rename_many_applies_a_swap_on_disk() {
        let d = scratch("rename_many_swap");
        fs::write(d.join("a.txt"), b"AAA").unwrap();
        fs::write(d.join("b.txt"), b"BBB").unwrap();
        let r = rename_many(
            d.to_string_lossy().to_string(),
            vec![
                RenamePair { from: "a.txt".into(), to: "b.txt".into() },
                RenamePair { from: "b.txt".into(), to: "a.txt".into() },
            ],
        )
        .unwrap();
        assert!(r.errors.is_empty(), "swap should apply cleanly: {:?}", r.errors);
        assert_eq!(fs::read(d.join("a.txt")).unwrap(), b"BBB");
        assert_eq!(fs::read(d.join("b.txt")).unwrap(), b"AAA");
        // No temp files left behind.
        let leftover: Vec<_> = fs::read_dir(&d)
            .unwrap()
            .filter_map(|e| e.ok())
            .filter(|e| e.file_name().to_string_lossy().starts_with(".cpe-rename-tmp"))
            .collect();
        assert!(leftover.is_empty(), "temp files leaked: {leftover:?}");
        // The returned undo reverses the batch.
        assert_eq!(r.undo.len(), 2);
        let _ = fs::remove_dir_all(&d);
    }

    #[test]
    fn rename_many_refuses_a_clobber_and_renames_nothing() {
        let d = scratch("rename_many_clobber");
        fs::write(d.join("a.txt"), b"A").unwrap();
        fs::write(d.join("keep.txt"), b"K").unwrap();
        // keep.txt is NOT in the rename set, so targeting it is a clobber → whole batch refused.
        let r = rename_many(
            d.to_string_lossy().to_string(),
            vec![RenamePair { from: "a.txt".into(), to: "keep.txt".into() }],
        );
        assert!(r.is_err(), "must refuse to clobber an out-of-set sibling");
        assert_eq!(fs::read(d.join("a.txt")).unwrap(), b"A");
        assert_eq!(fs::read(d.join("keep.txt")).unwrap(), b"K");
        let _ = fs::remove_dir_all(&d);
    }

    #[test]
    fn rename_many_rejects_duplicate_targets_and_bad_names() {
        let d = scratch("rename_many_dup");
        fs::write(d.join("a.txt"), b"A").unwrap();
        fs::write(d.join("b.txt"), b"B").unwrap();
        ...
        assert!(rename_many(
            d.to_string_lossy().to_string(),
            vec![
                RenamePair { from: "a.txt".into(), to: "same.txt".into() },
                RenamePair { from: "b.txt".into(), to: "same.txt".into() },
            ],
        )
        .is_err());
        // A path separator in a target is rejected.
        assert!(rename_many(
            d.to_string_lossy().to_string(),
            vec![RenamePair { from: "a.txt".into(), to: "sub/x.txt".into() }],
        )
        .is_err());
        // Nothing moved.
