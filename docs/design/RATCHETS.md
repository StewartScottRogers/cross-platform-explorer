...
| `dialog-body-reflow-allowlist` | `src/lib/dialogBodyReflow.test.ts` | scroll boxes in a centred dialog still allowed a content-driven height (CPE-1983) | 7 |
...
| dialog-body-reflow-allowlist | new → 7 | CPE-1983 | A brand-new baseline, so it has no value at the base revision and its first commit has to declare it. `src/lib/dialogBodyReflow.test.ts` enumerates every scroll box inside a centred dialog backdrop and requires a content-independent height, because a box that grows after an async load re-centres the dialog and slides every control above it out from under the pointer (CPE-1968; in `CheckpointDialog` the control the stray click can land on is `Revert…`). The sweep found 22 such boxes; 15 now have a stable height and 7 are listed. Every one of the 7 is a SECONDARY panel that is absent until the user asks for it — a drift list, a skips list, a collision list, a results pane, a command's output, the release notes, the column picker's list — where the reflow is the panel's ARRIVAL, which a fixed height cannot fix, and pinning one would only reserve dead space in the state the dialog spends most of its life in. That family needs a different fix (make an on-demand panel's arrival not move the dialog) and belongs in its own ticket rather than being smuggled in under this one; it is named in CPE-1983's report so it can be filed rather than forgotten. |
