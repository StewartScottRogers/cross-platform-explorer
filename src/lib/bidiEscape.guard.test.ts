...
const REGISTRY: Record<string, string[]> = {
  ...
  "AgentTimeline.svelte": ["605:agentName", "606:entries.length", "666:diff ? `${displaySafePath(e.path)} — hover to see what changed` : displaySafePath(e.path)", "671:KIND_LABEL[e.kind]", "673:stats.add", "673:stats.del", "674:clock(e.at)", "711:playing ? \"Pause\" : \"Play\"", "714:playing ? \"Pause\" : \"Play\"", "735:s", "761:cpMarkerTitle(m)", "762:`Checkpoint ${m.cp.label || cpShortId(m.cp.manifest_id)}`", "770:Math.round(sliderFraction(range, t) * 100)", "770:new Date(t).toLocaleTimeString()", "782:selectedCheckpoint.manifest_id", "783:selectedCheckpoint.label || cpShortId(selectedCheckpoint.manifest_id)", "784:cpTime(selectedCheckpoint.ts)", "790:revertPreviewError", "794:revertPreview.creates", "795:revertPreview.overwrites", "796:revertPreview.deletes", "797:formatBytes(revertPreview.bytes_written)", "799:revertPreview.drift_count", "803:revertPreview.drift_count", "803:revertPreview.drift_count === 1 ? \"\" : \"s\"", "817:revertError", "823:selectedCheckpoint.label || cpShortId(selectedCheckpoint.manifest_id)", "827:revertPreview.drift_count", "827:revertPreview.drift_count === 1 ? \"\" : \"s\"", "853:KIND_LABEL[replayCurrent.kind]", "855:clock(replayCurrent.at)", "906:replayKindLabel(re.kind)", "907:clock(re.ts)", "921:KIND_LABEL[e.kind]", "923:clock(e.at)", "944:c.sessionId", "947:formatTokens(c.inputTokens)", "948:formatTokens(c.outputTokens)", "949:formatTokens(c.totalTokens)", "950:formatUsd(c.costUsd)", "952:formatTokens(c.filesTouched)", "953:formatTokens(c.editCount)", "954:formatBytes(c.churnBytes)", "955:formatDuration(c.wallClockMs)", "959:formatPerMinute(c.tokensPerMinute)", "962:formatUsd(c.usdPerFile)", "965:formatBytes(c.churnPer1kTokens)", "989:relativeLabel(o.lastAt, Date.now())", "993:friendlyActor(a, sessions)", "1016:rc.kind === \"divergence\" ? \"diverged\" : \"collided\"", "1018:relativeLabel(rc.lastAt, Date.now())", "1022:friendlyActor(a, sessions)", "1025:renameConflictNote(rc.kind)", "1038:historyError", "1048:formatTokens(historyRollup.totals.sessions)", "1049:formatUsd(historyRollup.totals.costUsd)", "1050:formatTokens(historyRollup.totals.totalTokens)", "1051:formatDuration(historyRollup.totals.wallClockMs)", "1052:formatTokens(historyRollup.totals.filesTouched)", "1053:formatBytes(historyRollup.totals.churnBytes)", "1062:historyRollup.totals.sessions", "1062:historyRollup.totals.sessions === 1 ? \"\" : \"s\"", "1062:historyUnclean", "1063:historyUnclean === 1 ? \"its\" : \"their\"", "1064:historyUnclean === 1 ? \"is\" : \"are\"", "1065:historyUnclean === 1 ? \"it\" : \"them\"", "1074:formatPerMinute(historyRollup.ratios.tokensPerMinute)", "1077:formatUsd(historyRollup.ratios.usdPerSession)", "1080:formatUsd(historyRollup.ratios.usdPerFile)", "1083:formatBytes(historyRollup.ratios.churnPer1kTokens)", "1097:row.model", "1098:formatTokens(row.sessions)", "1099:formatTokens(row.totalTokens)", "1100:formatUsd(row.costUsd)", "1101:historyShare(row.costUsd, historyRollup.totals.costUsd)", "1117:row.agentName", "1118:formatTokens(row.sessions)", "1119:formatTokens(row.totalTokens)", "1120:formatUsd(row.costUsd)", "1121:historyShare(row.costUsd, historyRollup.totals.costUsd)", "1142:new Date(row.startedAt).toLocaleString()", "1143:row.agentName || row.agentId", "1143:row.agentName || row.agentId || \"(unknown)\"", "1144:historyDurationLabel(row)", "1152:isSessionEndedCleanly(row) ? \"Clean\" : \"Ended unexpectedly\"", "1186:historyMetric === 'cost' ? 'Cost' : 'Tokens'", "1198:historyBarDate(p.bucketStart)", "1198:historyMetric === \"cost\" ? formatUsd(v) : formatTokens(v)"],
  ...
  "CopilotDialog.svelte": ["197:planError", "200:phase === \"planning\" ? \"Planning…\" : \"Plan\"", "207:instruction", "214:v", "220:planResult.summary.moves", "220:planResult.summary.moves === 1 ? \"\" : \"s\"", "221:planResult.summary.renames", "221:planResult.summary.renames === 1 ? \"\" : \"s\"", "222:planResult.summary.deletes", "222:planResult.summary.deletes === 1 ? \"\" : \"s\"", "223:planResult.summary.mkdirs", "223:planResult.summary.mkdirs === 1 ? \"\" : \"s\"", "224:planResult.summary.copies", "233:opKind(op)", "244:execError", "268:v", "274:execResult.results.filter((r) => r.ok).length", "274:execResult.results.length", "281:r.error", "288:execResult.checkpoint.checkpoint.manifest_id", "293:undoing ? \"Undoing…\" : \"Undo\"", "296:undoError"],
  ...
  "TrashView.svelte": ["306:$t(\"trash.title\")","342:itemCountLabel","342:selectedCountLabel","344:$t(\"trash.stillLoading\")","344:selectedCountLabel","346:selectedCountLabel","360:$t(\"trash.moreActions\")","361:$t(\"trash.moreActions\")","380:allSelected ? $t(\"trash.deselectAll\") : $t(\"trash.selectAll\")","388:$t(\"trash.restoreSelected\")","396:$t(\"trash.emptySelected\")","403:$t(\"trash.emptyAll\")","409:$t(\"trash.refresh\")","413:$t(\"trash.docs\")","424:$t(\"trash.restoreFailed\", { name: displaySafeName(f.name), error: f.error })","432:$t(\"trash.loading\")","434:$t(\"trash.error\", { error })","470:noticeMessage","473:$t(\"trash.empty\")","519:degradedMessage","524:$t(\"trash.selectAll\")","526:$t(\"trash.columnsName\")","527:$t(\"trash.columnsOriginalPath\")","528:$t(\"trash.columnsDeleted\")","546:formatSize(e.size)","549:formatDate(e.time_deleted * 1000)","559:$t(\"trash.emptyConfirmTitle\")"],
  ...
  "ExplorerPane.svelte": ["521:$t(\"menu.view\")","523:$t(\"view.details\")","524:$t(\"view.list\")","525:$t(\"tb.icons\")","526:$t(\"view.gallery\")","530:$t(\"tb.sortBy\")","532:$t(\"sort.name\")","533:$t(\"tb.modified\")","534:$t(\"sort.type\")","535:$t(\"sort.size\")","539:$t(\"tb.direction\")","541:$t(\"cmd.ascending\")","542:$t(\"cmd.descending\")","546:$t(\"cmd.showHidden\")","553:$t(\"tb.fileList\")","589:$t(\"agent.watch\", { name: watchedAgentName })","591:baseName(c.path)","591:c.kind === \"removed\" ? \"−\" : c.kind === \"created\" ? \"+\" : \"~\"","591:c.path","594:$t(\"agent.watching\")","596:$t(\"agent.showLog\")","597:$agentTimeline.length ? `(${$agentTimeline.length})` : \"\"","597:$t(\"agent.log\")","604:selectedTag","605:visible.length"],
...
const APP_MARKUP_OFFENDERS = ["6487:$t(\"palette.openAgentBoardWindow\")","6499:$t(\"sidebar.repositories\")","6502:$t(\"sidebar.repositories\")","6508:$agentSessions.length === 0 ? $t(\"tb.openConsole\") : $agentSessions.length === 1 ? $t(\"tb.openConsoleOne\") : $t(\"tb.openConsoleMany\", { count: $agentSessions.length })","6516:$t(\"tb.aiConsole\")","6518:$agentSessions.length","6518:$t(\"tb.agentsRunning\", { count: $agentSessions.length })","6525:$t(\"tb.showDetailsPane\")","6530:$t(\"cmd.showHidden\")","6535:$t(\"cmd.folderSizes\")","6540:$t(\"tb.resetSettings\")","6627:$t(\"tb.paneWidth\")","6706:$t(\"tb.resizeNav\")","6707:$t(\"tb.resizeTip\")","6870:$t(\"tb.resizeDetails\")","6871:$t(\"tb.resizeTip\")","6880:$t(\"tb.popoutTip\")","6881:$t(\"tb.popoutAria\")","6886:$t(\"tb.defaultTab\")","6894:$t(\"tb.preview\")","6895:$t(\"view.details\")","6899:$t(\"tb.paneWidth\")","6915:$t(\"tb.previewOrDetails\")","6916:$t(\"tb.dragPopoutTip\")","6927:$t(\"tb.preview\")","6933:$t(\"view.details\")","7169:confirm.title","7184:passwordPrompt.title","7522:runConfirm.title","7540:macroParamPromptFor.macro.name","7806:$t(\"dnd.dropToImport\")"];
...
const APP_SCRIPT_BASENAME_ALLOWLIST = [2824, 2839];
