...
const REGISTRY: Record<string, string[]> = {
  ...
  "BackupDashboard.svelte": ["text:error","text:fmtTime(run.when)","text:fmtTime(st.when)","text:history[job.id].length","text:history[job.id].length === 1 ? \"\" : \"s\"","text:job.name","text:plan.copy.length","text:plan.createDirs.length","text:plan.delete.length","text:plan.skippedDirs.length","text:plan.unchanged","text:plan.update.length","text:progress","text:run.failed ? `, ${run.failed} failed` : \"\"","text:run.label","text:run.ok","text:showHistory === job.id ? \"▾\" : \"▸\"","text:st.failed ? `, ${st.failed} failed` : \"\"","text:st.label","text:st.ok","text:total ? ` / ${total}` : \"\""],
  ...
  // CPE-1891 round 4: preflight collision UI added blocked.length/confirmable.length/*Preview.more counts,
  // their "" : "s" pluralisers, the copy-button template literals, preflightError, the per-hazard `reason`
  // sentence (already stripped of its embedded path by genericizeReason — see this file's own comment),
  // CPE-1928's `blockedRemedy` (a frontend CONSTANT, no backend text in it at all), and
  // runLabel — none of these carry a filename/path, so they're recorded here rather than wrapped (the two
  // genuine path renders, `c.to` in both collision lists' text and title positions, are wrapped in
  // `displaySafePath` instead and so are absent from this list entirely). `text:running ? "Running…" : "Run"`
  // is gone because that ternary moved out of the render position into the `runLabel` reactive assignment.
  "MacroRunConfirm.svelte": ["text:blocked.length","text:blocked.length","text:blocked.length === 1 ? \"\" : \"s\"","text:blocked.length === 1 ? \"\" : \"s\"","text:blockedPreview.more","text:blockedRemedy","text:confirmable.length","text:confirmable.length === 1 ? \"\" : \"s\"","text:confirmable.length === 1 ? \"s\" : \"\"","text:confirmable.length === 1 ? \"this file\" : \"these files\"","text:confirmablePreview.more","text:copiedBlocked ? \"Copied\" : `Copy all ${blocked.length} name${blocked.length === 1 ? \"\" : \"s\"}`","text:copiedConfirmable ? \"Copied\" : `Copy all ${confirmable.length} name${confirmable.length === 1 ? \"\" : \"s\"}`","text:inputs.length","text:inputs.length === 1 ? \"\" : \"s\"","text:macro.name","text:macro.name","text:macro.steps.length","text:macro.steps.length === 1 ? \"\" : \"s\"","text:op.detail","text:op.input","text:op.kind","text:planError","text:preflightError","text:reason","text:run.ops.length","text:run.ops.length === 1 ? \"\" : \"s\"","text:runError","text:runLabel","text:undoError","text:undoing ? \"Undoing…\" : \"Undo\""],
...
const APP_MARKUP_OFFENDERS = ["6542:$t(\"palette.openAgentBoardWindow\")","6554:$t(\"sidebar.repositories\")","6557:$t(\"sidebar.repositories\")","6563:$agentSessions.length === 0 ? $t(\"tb.openConsole\") : $agentSessions.length === 1 ? $t(\"tb.openConsoleOne\") : $t(\"tb.openConsoleMany\", { count: $agentSessions.length })","6571:$t(\"tb.aiConsole\")","6573:$agentSessions.length","6573:$t(\"tb.agentsRunning\", { count: $agentSessions.length })","6580:$t(\"tb.showDetailsPane\")","6585:$t(\"cmd.showHidden\")","6590:$t(\"cmd.folderSizes\")","6595:$t(\"tb.resetSettings\")","6682:$t(\"tb.paneWidth\")","6761:$t(\"tb.resizeNav\")","6762:$t(\"tb.resizeTip\")","6925:$t(\"tb.resizeDetails\")","6926:$t(\"tb.resizeTip\")","6935:$t(\"tb.popoutTip\")","6936:$t(\"tb.popoutAria\")","6941:$t(\"tb.defaultTab\")","6949:$t(\"tb.preview\")","6950:$t(\"view.details\")","6954:$t(\"tb.paneWidth\")","6970:$t(\"tb.previewOrDetails\")","6971:$t(\"tb.dragPopoutTip\")","6982:$t(\"tb.preview\")","6988:$t(\"view.details\")","7224:confirm.title","7239:passwordPrompt.title","7577:runConfirm.title","7595:macroParamPromptFor.macro.name","7861:$t(\"dnd.dropToImport\")"];
...
const APP_SCRIPT_BASENAME_ALLOWLIST = [2863, 2878];
