...
const REGISTRY: Record<string, string[]> = {
  ...
  "BackupDashboard.svelte": ["139:job.name","149:progress","149:total ? ` / ${total}` : \"\"","153:fmtTime(st.when)","153:st.failed ? `, ${st.failed} failed` : \"\"","153:st.label","153:st.ok","167:history[job.id].length","167:history[job.id].length === 1 ? \"\" : \"s\"","167:showHistory === job.id ? \"▾\" : \"▸\"","174:fmtTime(run.when)","174:run.failed ? `, ${run.failed} failed` : \"\"","174:run.label","174:run.ok","192:error","196:plan.copy.length","196:plan.delete.length","196:plan.unchanged","196:plan.update.length"],
