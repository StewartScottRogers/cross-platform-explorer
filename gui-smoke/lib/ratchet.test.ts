...
const SEVEN_KNOWN_FAILING: KnownFailingFile = {
  specs: {
    ...
    "network.smoke.ts": { reason: "permanent Network section header never renders", ticket: "CPE-1595" },
...
/** The real, current `main` state: 33 pass, the 7 SEVEN_KNOWN_FAILING specs fail, all 40 report. */
function mainStateResults(): SpecResult[] {
  const knownFailingNames = new Set(Object.keys(SEVEN_KNOWN_FAILING.specs));
...
describe("evaluate — the known-7 baseline (current main state)", () => {
  it("is green: 33 pass, 7 known-failing, all 40 report", () => {
    const result = evaluate({
      results: mainStateResults(),
      knownFailing: SEVEN_KNOWN_FAILING,
...
describe("evaluate — clause 1: NEW GUI REGRESSION (an 8th spec fails)", () => {
  it("goes red when a spec outside known-failing.json fails", () => {
    ...
    const result = evaluate({ results, knownFailing: SEVEN_KNOWN_FAILING, expectedSpecCount: 40 });
...
describe("evaluate — clause 2: one-way ratchet (a known-failing spec starts passing)", () => {
  it("goes red when a listed spec passes, even though nothing else regressed", () => {
    const results = mainStateResults().map((r) =>
      r.spec === "network.smoke.ts" ? { ...r, status: "passed" as const } : r,
    );
    ...
    assert.equal(result.ok, false);
    assert.deepEqual(result.newFailures, []);
    assert.deepEqual(result.fixedButStillListed, ["network.smoke.ts"]);
    assert.equal(result.incomplete, false);
    assert.ok(
      result.messages.some(
        (m) => m.includes("RATCHET") && m.includes("network.smoke.ts") && m.includes("delete its entry"),
      ),
    );
  });

  it("passes clean once the fixed spec's entry is actually removed from known-failing.json", () => {
    const { "network.smoke.ts": removed, ...rest } = SEVEN_KNOWN_FAILING.specs;
    void removed;
...
describe("evaluate — clause 3: SUITE DID NOT COMPLETE (the CPE-1594 regression this exists to catch)", () => {
  it("goes red when fewer specs report than expected, even if every reported spec looks fine", () => {
    ...
    const result = evaluate({ results: truncated, knownFailing: SEVEN_KNOWN_FAILING, expectedSpecCount: 40 });
...
describe("evaluate — combined failure modes in one run", () => {
  it("reports incomplete + newFailures + fixedButStillListed all at once when all three happen together", () => {
    const results = mainStateResults()
      .map((r) => (r.spec === "network.smoke.ts" ? { ...r, status: "passed" as const } : r)) // ratchet fires
