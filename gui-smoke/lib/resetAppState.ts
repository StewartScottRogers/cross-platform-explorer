...
// relaunch — current folder, selection, an open dialog/menu, window size — now carries over between
// spec files unless something puts it back. This is that something: called once per spec file, from
// wdio.conf.ts's `beforeSuite` hook, BEFORE that file's own `before()`/`it()`s run.
//
...
// risk surface this ticket introduces is narrowly in-memory UI state, so that is all this resets.
 ...
 *   2. Escape, a few times — closes whatever dialog/menu/command-palette a prior spec left open. Every
 *      dialog component in this app closes on Escape (all 49 `src/lib/components/*Dialog.svelte` files
 *      reference it — the app-wide MENUS.md convention), including `ShredConfirmDialog`, so this is safe
 *      even after the one spec that must never click its own confirm button: Escape is not that button,
 *      and shred-dialog.smoke.ts never gets far enough to open a REAL destructive confirm regardless.
 *   3. `navigateTo(rootDir)` — the SAME address-bar navigation primitive `samples.smoke.ts` already uses
 ...
 *      than assuming Escape alone left the app somewhere sane. */
