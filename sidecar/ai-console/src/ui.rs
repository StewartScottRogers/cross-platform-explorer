//! The sidecar's own served UI (CPE-271, sidecar half).
//!
//! Per ADR 0001 each sidecar serves its own UI; the host embeds it in a frame. This is
//! a minimal, dependency-free HTTP server that serves one HTML page on an ephemeral
//! loopback port. The sidecar starts it after handshake and announces the URL to the
//! host (via a `Status` event), which then points an iframe pane at it (CPE-271, host
//! half). Loopback-only, so it isn't reachable off the machine.
...
mod tests {
    ...
    fn serves_the_page_over_a_loopback_port() {
        let server = serve("<h1>hello-ui</h1>".into()).unwrap();
