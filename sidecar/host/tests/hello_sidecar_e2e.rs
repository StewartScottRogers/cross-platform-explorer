...
fn hello_exercises_all_four_capabilities_over_a_real_process() {
    ...
    let outcome = { let _tok = conn.launch_token().to_string(); handshake(&mut conn, CONTRACT_VERSION, &all_caps(), "hello", Some(&_tok)) }.expect("handshake");
...
fn hello_exits_zero_on_clean_shutdown() {
    ...
    let outcome = handshake(&mut conn, CONTRACT_VERSION, &all_caps(), "hello", None).expect("handshake");
