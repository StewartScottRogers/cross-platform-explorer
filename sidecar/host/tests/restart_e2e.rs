...
fn a_crashed_sidecar_is_detected_and_restarted() {
    ...
    { let _tok = conn.launch_token().to_string(); handshake(&mut conn, CONTRACT_VERSION, &consent, "echo", Some(&_tok)) }.expect("handshake");
    ...
    let outcome = { let _tok = restarted.launch_token().to_string(); handshake(&mut restarted, CONTRACT_VERSION, &consent, "echo", Some(&_tok)) }.expect("re-handshake");
