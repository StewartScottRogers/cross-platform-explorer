---
id: CPE-1685
title: "Route s3 through cpe_vfs::open â€” AccessKey credentials from the keychain, and the missing-secret guard"
type: Feature
status: Backlog
priority: Medium
component: Backend
...
connection browses through the same CPE-1511 command routing SFTP, WebDAV and FTP already use â€” with no
...
- An `s3` arm in `cpe_vfs::open` building an `S3Config` from the `Connection` and handing back a boxed
  `S3Provider`. ~~`conn.host` is the bucket (`location.rs` already parses `s3://bucket/key` that way â€”
  check it, do not re-derive it), `conn.path` the prefix.~~ **âš  SUPERSEDED by CPE-1686 â€” do not build
  ...
  below â€” read that section before writing any of this ticket.
- `AuthMethod::AccessKey { id, secret_ref }` â†’ the access key id and, from the keychain, the secret. Note
  that `secret_ref` is currently **declared and used nowhere** â€” this is its first consumer, so decide
...
- Endpoint and region: where do they come from for a saved connection? AWS can be inferred from the
  region, but MinIO/B2/Wasabi cannot. Resolve this â€” a `path` convention, a new optional field, or a
  documented default â€” and state the answer in the ticket. CPE-1686 needs to know what the form collects.
...
- The three sibling providers' `AccessKey` error messages say "reserved for a **future** S3/cloud provider".
  It is no longer future â€” reword them.
...
- [ ] The `unsupported scheme 's3'` test in `crates/vfs/src/lib.rs` is replaced â€” not deleted â€” by one
...
- [ ] An `AccessKey` connection with no stored secret fails with a message naming the missing secret, and
      **no HTTP request is issued** â€” asserted, so a future refactor cannot quietly reintroduce a
...
- [ ] `default_port("s3")` returns a sensible value and the connection's derived location string matches
      what `src/lib/network.ts`'s `DEFAULT_PORTS` will produce (CPE-1686 keeps the two in sync â€” that
      mirroring is deliberate and already documented in `network.ts`).
- [ ] `default_port("s3")` returns **443**, pinned by a Rust test â€” the TS side asserts the same literal
...
## Handed over from CPE-1686 (frontend landed first, 2026-08-12) â€” read before deciding
...
- **`host` = the endpoint host** (`s3.us-east-1.amazonaws.com`, `minio.lan`), **`port` = the endpoint port**,
  **`user` = the region** (blank â‡’ `us-east-1`), **`path` = `/bucket[/prefix]`** with the bucket as the first
  ...
  those endpoints cannot be derived from a region. `location.rs` still parses the result unchanged â€”
...
- **`secret_ref` = the connection's `name`** â€” the same key `secret_for(access, &conn.name)` already uses. It
  is a label naming the keychain entry, never the secret. That is what the form now writes.
- **`default_port("s3")` must be `443`** â€” `network.ts`'s `DEFAULT_PORTS.s3` is 443 and `network.test.ts`
...
- The frontend already refuses to connect an `AccessKey` connection with no stored secret
  (`secretAlwaysRequired`), prompting for "Secret access key" first â€” the backend guard this ticket adds is
...
- `src/docs/31-network.md` carries a transitional paragraph ("saving works now; the provider ships alongside
  it"). **Delete that paragraph as part of this ticket** â€” once `s3` routes, it becomes a lie.
- **And flip the tense with it.** The "Honest limits of object storage" list and the `## Limits` section
  ...
  tense, which told a reader that S3 browsing worked with caveats when no S3 code path existed at all â€”
...
## BLOCKED-BEHIND CPE-1704 â€” do not land this before it
...
paths, SFTP and WebDAV. The security property holds â€” nothing can escape the listed prefix â€” but it imports
...
plainly in the PR that you are shipping a known silent-hide â€” do not land it quietly.
...
0-byte file on Windows -- the bytes go into an NTFS alternate data stream and the user sees an empty
file named only the part before the colon. Measured through the real sink, not inferred. It is a
data-integrity bug, not a security one (traversal was checked and does not escape).
