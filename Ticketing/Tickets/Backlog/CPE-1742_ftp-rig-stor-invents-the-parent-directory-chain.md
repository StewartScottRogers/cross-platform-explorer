...
Measured while filing: removing the `create_dir_all` leaves `cpe-ftp` **13/13 green**, and this crate
has no `upload_tree` test at all — so nothing *in this crate* depends on it. That is a statement about
...
Filed from CPE-1731 (PR #905), where the deliberate non-change is recorded at the site. Related:
