---
id: CPE-1482
title: "Add /sprint-batched — a bounded sprint wrapper with a max-batch-count stop condition"
type: Feature
status: Done
priority: Medium
...
---
## What
A new `.claude/commands/sprint-batched.md` skill: the autonomous `/sprint` loop, but **bounded by a
max batch count** so it can be started before leaving (work / sleep) and winds down cleanly on its own
instead of running indefinitely. Requested by the user 2026-08-08 ("a real sprint-batch skill with a
...
- **`max_batches` ceiling** (default 40; sizing table for sleep/workday/long-day in the skill). Persisted
  counter at `.claude/sprint-metrics/BATCH-COUNTER` that **survives session-resets** (the resuming
...
`.claude/commands/` (only `sprint.md` present) or the concurrent `sprints_*` family (CPE-1476);
references `sprint.md` + PURPOSE.md correctly; defers to `sprint.md` on any conflict except the bound.

## Work Log
- 2026-08-08 (sprint, Foreman-authored): wrote `.claude/commands/sprint-batched.md`; filed + closed
