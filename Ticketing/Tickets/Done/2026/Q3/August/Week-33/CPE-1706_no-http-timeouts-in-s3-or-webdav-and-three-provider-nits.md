...
### 1. No HTTP timeouts â€” wall-clock is completely unbounded *(the real one)*

`crates/s3/src/provider.rs:299` builds its agent as `AgentBuilder::new().redirects(0).build()` â€” no
`timeout_read`, no `timeout_write`, no `timeout`. **`ureq` 2.x defaults all three to `None`.**

`crates/webdav/src/lib.rs:61` does the same. This is inherited, not introduced by CPE-1683 â€” but S3 is
...
indefinitely, and the caps compound it: a hostile server gets up to 1000 Ã— RTT of held thread with no
...
entire document â€” so an `<IsTruncated>` or `<NextContinuationToken>` nested inside a `<Contents>` element
...
A hostile server can return a single key of ~8 MiB â€” the body cap is the only limit. **Real S3 caps keys
...
`.github/workflows/ci.yml:419-422` still says the s3 slice *"is pure computation â€¦ there is nothing to run
against and no fixture server."* CPE-1683 added a `tiny_http` fixture binding `127.0.0.1:0`. The
*conclusion* is still right â€” it needs no network, services or Docker â€” but the stated reason is wrong.
...
there is no flake risk today â€” this is purely about what happens the day someone breaks it.
...
happened. **This is the only test with that property** â€” `MAX_LIST_ENTRIES` and the truncation guard both
red cleanly in ~10 s, and the depth guard's break crashes fast and loud â€” so it is one test, not a pattern.
...
is transcription â€” a fixture returning an over-cap body, asserting that error.
...
- [ ] Both agents set `timeout_read` and `timeout_write` (and consider an overall `timeout`). **Pick the
      values deliberately and record why** â€” too short breaks a legitimately slow large listing over a
...
- [ ] A test proves the timeout fires: a fixture that accepts the connection and then stalls must produce
      ...
      timeout is later removed â€” a test whose failure mode is a six-hour CI timeout is not a good test.
...
- [ ] Item 5: the page-cap test cannot hang. Prove it â€” break the page cap and show the test produce a
...
**2026-08-13** â€” Worked on branch `cpe-1706-http-timeouts`. All six items done; nothing deferred.

### Item 1 â€” the values chosen, and why
...
while `timeout_connect` already defaults to 30 s (`agent.rs:256`) â€” so **connect was the one phase that was
...
- `TIMEOUT_READ` / `TIMEOUT_WRITE` = **30 s**, in both crates. These are *per read/write*, not per request:
  ...
  That is the property a large listing (or a large `read`) over a poor link needs â€” it may take as long as
  it takes provided it keeps moving. 30 s is a wide margin over any real gateway's time-to-first-byte
  (AWS's own SDKs use 30â€“60 s for the same knob).
...
- **`ureq`'s overall `.timeout()` is deliberately NOT set**, in either crate. Two reasons, both checked in
  ...
  multi-minute GET of a large object over a bad connection â€” a real user, not a hypothetical; (b) it
  *takes precedence over* `timeout_read`/`timeout_write` (`agent.rs:476-477`) rather than adding to them,
  ...
  per-**listing**, not per-**request** â€” see next.
- `MAX_LIST_WALL_CLOCK` = **10 min**, `cpe-s3` only. This is the bound a per-read timeout *cannot* give and
  ...
  29 s never trips `timeout_read`, and `MAX_LIST_PAGES` then multiplies that â€” 1000 pages Ã— a 30 s stall is
...
`build_agent` â†’ `list`/`read` â†’ `req.call()` path `connect` drives â€” only the `Duration`s differ â€” because
...
- **2.** Page-level `IsTruncated`/`NextContinuationToken` now read from `root_element().children()`, and
  `Key`/`Size`/`Prefix` from their own container's `children()` â€” plus `Contents`/`CommonPrefixes`
...
- **5.** `call_with_deadline` added; `a_server_that_never_stops_truncating_is_capped_by_max_list_pages`
  routed through it at 60 s (~235Ã— the green path's measured 255 ms). Proven: with `MAX_LIST_PAGES` raised
...
- **6.** An over-cap body is pinned to the honest parse error `s3: bad ListObjectsV2 XML: the root node was
  opened but never closed` â€” exactly the string the ticket predicted. The fixture keeps its keys under
...
**2026-08-13, round 2** â€” a Reviewer (from the code) and an independent UAT (by measurement) each found
...
at all â€” measured holding a `list` thread past 100 s at shipped values, in both crates. `timeout_read` is
...
and PR body both claimed. WebDAV was the live half â€” it is routed through `crates/vfs` today, inside
...
`ureq::Request::timeout` â€” **per request**, distinct from the agent-level `.timeout()` this ticket
...
(`stream.rs:433-441` is a genuine either/or), so the value must also keep a dead share failing promptly â€”
120 s bought no real listing any safety and made a dead endpoint take two minutes to report.

Corrected worst case for one `list`: `MAX_LIST_WALL_CLOCK + TIMEOUT_LIST_REQUEST` â‰ˆ **12 minutes**, since
...
- **Item 6 was overclaimed.** An over-cap body whose truncation lands after a complete root element is
  still well-formed and parsed as a short, *complete-looking* listing â€” the UAT reproduced
...
- **Eight guard arms reddened nothing.** Now guarded â€” and two of my first attempts at guarding them were
...
- `WebdavProvider::read` gains a byte cap that **errors rather than truncating** â€” `download_tree` writes
...
**Closed 2026-08-13, merged as PR #892 (`ceaebb12`).** Two rounds. Round 1 shipped five well-guarded items
sitting on top of an **unfixed headline bug**, with the code asserting it was fixed.

### Round 1 bounded three stall shapes and missed the fourth â€” the one the ticket opens with

Per-read timeouts (`timeout_read`/`timeout_write`, 30 s) genuinely closed: accept-then-silence (29.99 s),
dead mid-body (30.01 s), stall between pages (30.01 s), WebDAV never-answer (30.01 s). All measured.

They did **nothing** for the dribble â€” valid `200 OK` headers, then one byte every 5 s:

```
S3Provider::list at SHIPPED values ... did not return within 100s.
WebdavProvider at SHIPPED values ... did not return within 100s.
```

**Cause:** `MAX_LIST_WALL_CLOCK` was evaluated at the *top of the page loop, before `signed_get`*, so it
could not fire mid-body; `timeout_read` is per-read and its clock restarts on every byte. Nothing bounded
one page's body. Worst case against the 8 MiB cap at one byte per 29 s â‰ˆ **7.7 years** on a held blocking
thread â€” while the PR body disclosed a residual of "ten minutesâ€¦ and it is bounded, which is the point".

**WebDAV was live** (S3 is latent until CPE-1685): `crates/vfs/src/lib.rs:56` â†’ `remote_dir_entries` â†’
`remote_list_dir_impl` â† `list_dir`, on `spawn_blocking` with tokio's default 512-thread pool and no
`max_blocking_threads`. And `read` was `read_to_end` with no `.take()` â€” unbounded in memory too.

**Two independent confirmations.** The UAT measured it with raw-TCP fixtures; the Reviewer reached the same
conclusion **from code reading, before seeing the UAT's report**.

### The fix, and the distinction that made it possible

The author correctly declined the **agent-level** `.timeout()` after reading the vendored source:
`stream.rs:433-441` is a genuine either/or â€” with it set, `timeout_read` is *never consulted*, so it would
have traded a bound rather than added one. Right call.

The mistake was flattening "which request deserves which bound" into one all-or-nothing choice. `ureq`'s
**per-request** `Request::timeout` (`request.rs:122`; `DeadlineStream::fill_buf` at `stream.rs:85-89`
recomputes the remaining budget on *every* read) bounds a body continuously. Applied to the ListObjectsV2
GET and WebDAV's PROPFIND/MKCOL/DELETE/MOVE â€” deliberately **not** to the large-object GET or PUT, where
per-read semantics are what a legitimately slow large transfer needs.

Measured after: dribble cut at **60.009 s** (S3 `list`), **60.003 s** (WebDAV `list` and `stat`); the
legitimate slow-but-progressing case still returns **`Ok` in 9.51 s**.

### Honest worst case, and the residual that stayed

`MAX_LIST_WALL_CLOCK + TIMEOUT_LIST_REQUEST` = 600 + 60 = **660 s â‰ˆ 11 min**, because a final page can
start just under the budget and consume its own full deadline. Both bounds are load-bearing: without the
budget, 1000 Ã— 60 s = 16.7 hours.

**WebDAV `read` remains time-unbounded against a dribbler** (measured past 150 s), capped only in memory at
2 GiB. Deliberate: an end-to-end deadline would kill a legitimate multi-minute download for being slow, and
`read` needs a user to ask for it, unlike `list` which the app issues on every navigation. Now *stated* in
the doc rather than left to inference.

**A new cliff, judged rather than waved at:** a page making continuous progress at one byte per 500 ms is
now killed at 60 s. Against real page sizes â€” typical (1000 keys, ~90 KB) needs >12 kbit/s; long-key
(~350 KB) needs >47 kbit/s; pathological (1000 Ã— 1024-byte keys, ~1.1 MB) needs >147 kbit/s. Only the last
is reachable on a congested 2G or bad satellite link, and it fails as a clear retryable error naming the
endpoint. Errs slightly aggressive, correctly so.

### Eight dead arms, and one that defeated its own purpose

The Reviewer enumerated rather than spot-checked: both `timeout_write`, both `timeout_connect`, three
`descendants()` container filters, and the error-path body cap all redded **nothing**. `TIMEOUT_CONNECT`
was the sharpest â€” its stated purpose was to pin against a future `ureq` default change, but nothing tested
it was *wired*, so it could not do the job it was added for. Now `build_agent` takes it as a parameter and
the test asserts `Some(13s)`, so deletion surfaces as ureq's `Some(30s)` default and is caught.

Item 6 moved from shape-inference to **length-based** (`take(cap+1)`, compare), closing the UAT's
counter-example: a well-formed root element followed by legal post-root whitespace previously returned `Ok`
with one entry â€” a truncated listing sold as complete.

### Foreman fixes applied at merge

Three comments asserting bounds the code did not have â€” the exact defect this ticket exists to eliminate:
the module doc still said `TIMEOUT_LIST_REQUEST` was 120 s (with "33 hours" and "12 minutes" derived from
it); the test-section comment still claimed the guard refuses `..%2f` when a test 80 lines below asserts it
is **accepted**; and the WebDAV doc named a mechanism while leaving its consequence to inference.

### Also handled

The author **trial-merged rather than trusting MERGEABLE** and found a genuine two-hunk conflict with
CPE-1704 that both PRs reported as clean. During resolution they folded `MAX_KEY_LEAF_BYTES` into
`is_safe_s3_leaf` as another arm rather than a separate `continue`, so over-long keys are now *counted*
into `filtered_count` instead of vanishing â€” closer to CPE-1704's point than the ticket's own wording.

**Cost logged, not gated:** the green suite went 1.20 s â†’ 60.02 s (s3) and 0.61 s â†’ 60.01 s (webdav), Ã—3
OSes â‰ˆ 6 min per CI run, because each crate now waits out its **real shipped deadline** instead of an
injected one. Both checks flagged it and both endorsed the trade â€” testing through an injected seam is
exactly what let round 1 ship a configuration that did not bound.

Verdicts: UAT **PASS**, Reviewer **CHANGES REQUESTED** on the three comments only, with explicit support for
fixing and merging without a further round. All 13 CI checks green across the 3-OS matrix.
