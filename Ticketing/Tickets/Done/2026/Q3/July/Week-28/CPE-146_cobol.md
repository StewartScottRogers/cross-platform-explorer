---
id: CPE-146
title: Preview/edit support for COBOL files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a preview provider for COBOL (.cbl/.cob) in the right-side preview pane. Highlighted COBOL source. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [x] .cbl/.cob is matched by a dedicated preview provider in the bundled registry
...
- [x] Viewer: Highlighted COBOL source.
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js grammar: cobol (3rd-party). Editing model: source.
Syntax highlighting builds on [[CPE-065]]; editable types reuse [[CPE-066]] write_file_text.
## Resolution

Delivered .cbl/.cob/.cpy preview/edit: mapped to the `code` category. No COBOL grammar ships with highlight.js in this build, so per the AC they use the escaped-monospace fallback. Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts + tests.
