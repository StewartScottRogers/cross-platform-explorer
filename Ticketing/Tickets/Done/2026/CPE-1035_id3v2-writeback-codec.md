...
`apply_edits`→write→read chain, audio preserved byte-for-byte, Clear path, garbage inputs). Clippy clean
both feature modes. First write-back codec — the studio is now editable for audio. Reviewer noted a
benign symmetric limitation (v2.4 footer on an *incoming* tag not stripped — reader has the same gap).
