...
- [ ] .odt is matched by a dedicated preview provider, registered in the bundled provider registry
- [ ] Viewer: Render the document body for reading.
- [ ] Editing: Read-only viewer — this format is not practically editable in place; falls back to metadata for edit intent.
- [ ] Backend support: Backend unzip + content.xml transform — lands green via CI (Rust builds/tests locally now too)
