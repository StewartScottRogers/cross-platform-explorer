...
## Acceptance Criteria
- [ ] Double-clicking an Agents leaf opens the AI Console and **activates that session's tab** (matched
      by `sessionId`, reusing the CPE-490 correlation).
- [ ] The Agents context menu (`AgentMenu.svelte`) gains an **"Open"** item that opens the console to the
      same tab (shown alongside the existing close items + the session chip).
- [ ] If the AI Console window is **already open**, it is **focused and switched** to the right tab —
      not reopened / not disrupting other sessions.
- [ ] If the console is closed, it opens and activates the tab once its sessions have (re)attached.
- [ ] Graceful when the session has ended / no matching tab (a clear notice, no crash).
- [ ] Existing single-click behaviour (navigate the explorer to the agent's folder) is preserved;
