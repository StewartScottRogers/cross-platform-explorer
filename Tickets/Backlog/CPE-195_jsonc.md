---
id: CPE-195
title: Preview/edit support for JSON with comments files
type: Feature
status: Open
priority: Low
component: Frontend
...
Add a preview provider for JSON with comments (.jsonc) in the right-side preview pane. Highlighted JSONC. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [ ] .jsonc is matched by a dedicated preview provider in the bundled registry
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js grammar: json. Editing model: source.
