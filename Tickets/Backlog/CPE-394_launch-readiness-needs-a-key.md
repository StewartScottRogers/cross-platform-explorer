...
## Acceptance Criteria
- [ ] `providerNeedsKey()` (not native / not a local provider) + `haveKeyFor()` (saved credential or
      typed key).
- [ ] On provider change / load: if it needs a key and none is set, a clear inline hint with a
      "Add a key" action (or "pick a built-in-login provider").
- [ ] Default provider prefers one that needs no key when the user has no keys yet.
- [ ] Harness tests: hint shows/hides correctly; no-key provider chosen as default when keyless.
