...
- [ ] .pem/.crt is matched by a dedicated preview provider, registered in the bundled provider registry
- [ ] Viewer: Decode and show subject, issuer, validity, fingerprints.
- [ ] Editing: Read-only viewer — this format is not practically editable in place; falls back to metadata for edit intent.
- [ ] Backend support: Backend X.509 parse — lands green via CI (Rust builds/tests locally now too)
