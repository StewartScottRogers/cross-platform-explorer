...
- [ ] `ConsoleState.registry` becomes hot-swappable (behind a lock); all read sites updated.
- [ ] `ConsoleState` remembers its catalog dir + trusted keys so a reload re-runs
      `load_signed_source` over the bundled dirs + verified source.
- [ ] `reload_catalog()` + `POST /api/catalog/reload` → re-scan and swap the registry atomically;
      returns the new agent count.
- [ ] Tests: after adding a newly-signed manifest to the source dir, reload surfaces it; a bad
      source leaves the current registry intact.
- [ ] clippy clean; no regression to launch/catalog/install paths.
