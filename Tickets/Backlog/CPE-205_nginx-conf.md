---
id: CPE-205
title: Preview/edit support for Nginx configuration files
type: Feature
status: Open
priority: Low
component: Frontend
...
Add a preview provider for Nginx configuration (.nginx) in the right-side preview pane. Highlighted Nginx config. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [ ] .nginx is matched by a dedicated preview provider in the bundled registry
...
- [ ] Viewer: Highlighted Nginx config.
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js grammar: nginx. Editing model: source.
