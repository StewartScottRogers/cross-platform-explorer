---
id: CPE-150
title: Preview/edit support for Zig files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a preview provider for Zig (.zig) in the right-side preview pane. Highlighted Zig source. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [x] .zig is matched by a dedicated preview provider in the bundled registry
...
- [x] Viewer: Highlighted Zig source.
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js grammar: zig (3rd-party). Editing model: source.
Syntax highlighting builds on [[CPE-065]]; editable types reuse [[CPE-066]] write_file_text.
## Resolution

Delivered .zig preview/edit: mapped to `code`; no Zig grammar ships (third-party only), so escaped-monospace fallback per the AC. Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts + tests.
