...
- [x] .vue is matched by a dedicated preview provider in the bundled registry
...
Delivered .vue preview/edit: mapped to `code` and aliased to the `xml` grammar (an SFC is HTML-based markup), giving reasonable highlighting of the template/markup. Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts, src/lib/preview/highlight.ts + tests.
