---
id: CPE-181
title: Preview/edit support for Astro components files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a preview provider for Astro components (.astro) in the right-side preview pane. Highlighted Astro. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [x] .astro is matched by a dedicated preview provider in the bundled registry
...
Delivered .astro preview/edit: mapped to `code` and aliased to the `xml` grammar (Astro components are HTML-based). Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts, src/lib/preview/highlight.ts + tests.
