---
id: CPE-092
title: Preview/edit support for iCalendar (ICS) files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a first-class preview provider for iCalendar (ICS) (.ics) in the right-side preview pane.
List of events with times and summaries. Read-only viewer — this format is not practically editable in place; falls back to metadata for edit intent.

## Acceptance Criteria

- [x] .ics is matched by a dedicated preview provider, registered in the bundled provider registry
- [x] Viewer: List of events with times and summaries.
