---
id: CPE-185
title: Preview/edit support for Liquid templates files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a preview provider for Liquid templates (.liquid) in the right-side preview pane. Highlighted Liquid. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [x] .liquid is matched by a dedicated preview provider in the bundled registry
...
Delivered .liquid preview/edit: mapped to `code` and aliased to the `handlebars` grammar (Liquid shares handlebars-style {{ }} / {% %} tag syntax). Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts, src/lib/preview/highlight.ts + tests.
