...
- [x] .bmp is matched by a dedicated preview provider, registered in the bundled provider registry
- [x] Viewer: Show via the native image element.
...
Delivered: .bmp is classified as an image and rendered by the image provider via <img> (native). Verified + regression-tested. Load cancellation and large/corrupt-file fallback come from the shared PreviewPane (CPE-059).
