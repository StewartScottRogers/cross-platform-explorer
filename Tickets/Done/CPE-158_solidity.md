---
id: CPE-158
title: Preview/edit support for Solidity files
type: Feature
status: Done
priority: Medium
component: Frontend
...
Add a preview provider for Solidity (.sol) in the right-side preview pane. Highlighted Solidity source. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [x] .sol is matched by a dedicated preview provider in the bundled registry
...
- [x] Viewer: Highlighted Solidity source.
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js grammar: solidity (3rd-party). Editing model: source.
Syntax highlighting builds on [[CPE-065]]; editable types reuse [[CPE-066]] write_file_text.
## Resolution

Delivered .sol preview/edit: mapped to `code`; no Solidity grammar ships (third-party only), so escaped-monospace fallback per the AC rather than a misleading C/JS alias. Editable as source via the shared text provider (write_file_text, CPE-066); load cancellation + large/corrupt-file fallback come from the shared PreviewPane (CPE-059). Files: src/lib/filetypes.ts + tests.
