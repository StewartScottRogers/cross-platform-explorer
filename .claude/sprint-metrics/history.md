...
## 2026-08-28 — six narrow reviews, six blockers, zero code defects

PR #1087 has now been through six narrow re-reviews, each scoped to only what the previous round
changed. **Every one found a blocking defect. Not one was a code defect.** The sixth reviewer says it
outright: *"The code in this round is sound… The findings below are claim-scope defects, all in the same
shape the round exists to eliminate."*

The six, in order: a gap list that said *all* and had a member that didn't; a red-proof naming a mutation
the code cannot express; a quantifier over JavaScript backed by a three-row table; a limit sentence
refuted by 38 characters; a sweep whose 36,861 was 120 programs; and now a committed tool whose own
triage guidance points at a table it **provably cannot intersect** — the corpus keeps only parseable
inputs, the table's derived invariant is that none of its entries parse, so all 40 findings are valid
JavaScript being corrupted and the guidance says they're expected.

**Six rounds is enough data to stop treating these as separate mistakes.** They are one mistake made by
one mechanism: **the author of a fix knows the mechanism better than anyone alive at that moment, and
that knowledge produces confident sentences about what it cannot do.** Those sentences are generated by
the same reasoning that produced the fix, so they inherit its blind spots and get none of its testing —
and they land in the same commit as a green suite, which reads as vouching for them.

**Two things follow, and I'm putting both into how this PR is briefed from here.**

**Do not write a sentence you have not measured, and prefer deleting a claim to narrowing it.** Narrowing
is what produced rounds 4, 5 and 6 — each replaced a false sentence with a more specific false sentence.
Deletion has no failure mode.

**And where a claim ranges over something enumerable, derive the enumeration in code.** This round has
the cleanest example yet: round 6 added an assertion listing *which tables the oracle sweeps*, precisely
so a family held back would be visible rather than absent. **The list is hand-written.** Registering a
new family in neither the table-list nor the sweep leaves the suite 65/65 green — so it catches the half
where you remember, and misses the half it was built for. The file already imports `readFileSync`;
scanning its own source for the declarations closes it in a few lines. **A recalled enumeration inside a
guard against recall is the whole shift in one line of code.**

Worth recording what did *not* go wrong, because it is the reason none of this shipped: **the mitigation
held every single time.** The checked wrapper throws on all eight newly-found deleting inputs, the
