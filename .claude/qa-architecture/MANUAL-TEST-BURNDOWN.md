...
| 8 | **Native OS metadata interop** (ADS on Win, xattr on Linux) verified with OS tools | `native_tags_demo` prints values; human runs `Get-Item -Stream` / `getfattr` to confirm | 🔧 in progress | Make the example **self-assert** by re-reading via the OS tool and comparing, then run it in the 3-OS matrix | _unfiled_ |
