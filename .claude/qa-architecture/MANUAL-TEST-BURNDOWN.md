...
**MVD (still-manual surfaces): 8 primary + 9 supplementary = 17 total** · _baseline seeded 2026-07-25; row #8 flipped ✅ (CPE-1049); row #6's download/verify sub-surface automated (CPE-1058) — row stays in MVD for the still-attended in-place binary swap; row #9 added 2026-07-29 (CPE-1129 UAT deferred the standalone-board switcher's live-browser click-through) then flipped ✅ 2026-07-31 (CPE-1168 headless click-through); **row #5 flipped ✅ 2026-08-04 (CPE-1307 — macOS `xattr` OS-interop test, confirmed on the macos-latest CI leg), 7→6**._
...
| 7 | **Real remote network run** (non-loopback client↔server over the wire) | loopback via `cpe-net` example + unit tests | 🔧 in progress | Containerised two-host network E2E in CI (server container + client), asserting listing over a real socket. **Now designed and filed as slice 2 of CPE-1659** — build `crates/net/src/bin/cpe-server-ref.rs` into a container on the job's Docker bridge network and drive it at its container IP (a different network namespace, non-`127.0.0.1`), reusing the same rig row #13 needs | CPE-819/820 → CPE-1659 |
...
| 12 | **AI search dialog end-to-end** (shipped v0.57.45; query → results → navigate on a real build) | logic covered by jsdom component tests; **no `gui-smoke` spec** — the dialog opens via the command palette rather than a free key combo, the same obstacle recorded for `ContentIndexSearchDialog` (CPE-1263) | ⛰ manual | A `gui-smoke` spec that drives the palette (`Ctrl+Shift+P` → type → Enter) to open it, asserts a real ranked result renders over live IPC, and `snap()`s it in both themes. Solving palette-driven opening once also retires the CPE-1263 residual — build the palette helper into `gui-smoke/lib/` | — (owed to user; unfiled) |
| 13 | **Remote providers against a REAL server** (SFTP / WebDAV / FTP / FTPS interop: transfer modes, href encoding, host-key TOFU vs a real sshd) | **only against servers we wrote ourselves** — `crates/sftp` an in-process `russh-sftp` server, `crates/webdav` a `tiny_http` handler, `crates/ftp` a hand-rolled daemon. `ci.yml` states this explicitly three times. The fixture agrees with the client by construction, so no interop bug is reachable | 🔧 in progress | **CPE-1659**: a Docker rig on `ubuntu-latest` (real OpenSSH `sftp-server`, Apache `mod_dav`, vsftpd) driven through the real `cpe_vfs::open` routing seam by one shared conformance fn, with mutations verified from the **host-mounted directory on disk** (the CPE-1049/1307 "OS's own tools" pattern) and a required negative control. Shrinks CPE-1518 to its device-specific residue and gives the SMB epic (CPE-1504) a landing pad | CPE-1659 |
