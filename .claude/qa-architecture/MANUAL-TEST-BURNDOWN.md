...
| 6 | **Auto-update flow** (updater downloads, verifies signature, swaps in place) | none end-to-end | 🔧 in progress | Hermetic `crates/updater-verify` unit tests (manifest shape + minisign signature verify against configured pubkey + version match) on the 3-OS `Server crates` matrix, plus a `release.yml` guard over the real built artifacts; in-place binary swap stays attended | CPE-1058 |
