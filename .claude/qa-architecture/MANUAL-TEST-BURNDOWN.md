...
**MVD (still-manual surfaces): 6 primary + 8 supplementary = 14 total** · _**derived, not asserted** —
...
_**2026-08-26 (CPE-1882):** **row #3 half-closed, not flipped.** This ticket's whole premise was that
nothing local could answer a *layout* question — jsdom applies no component CSS, so no unit test in
this repo could ever check overlap/clipping/on-screen position, which is exactly why row #3's "pill
reflow, light+dark" clause has sat at 🔧 since baseline. `scripts/dev-harness/layout-guard/` (generalised
from the CPE-1659/1859/1836 `statusbar-notice` prototype, reusing `sidebar-drop-stack-overlap`'s
CDP-over-`chrome.exe --headless=new` shape — no WebDriver) is now wired into `.github/workflows/gui-smoke.yml`
as its own **blocking, unconditional** job (`layout-guard`): real `getBoundingClientRect()` +
`elementFromPoint()` measurements against the real `StatusBar.svelte`/`TrashView.svelte`, asserting no two
elements in a row overlap, no text paints past its own background, and no clip-relevant control becomes
unreachable — the exact pill/chip-reflow rule row #3 names, intended to be enforced by CI rather than
described in a CLAUDE.md convention nobody re-checks. Red-proofed against both CPE-1836 (status bar
git-block bleed) and CPE-1827 (Trash titlebar clip at the 600px floor) — see the ticket's own Work Log
for the full before/after output (LOCAL at this point in the shift — see the correction immediately
below for the CI-only gap this pass missed). **This closes the LAYOUT half of row #3** (overlap/clip/
text-escape, geometry-only, 2 components today, extensible via `cases.mjs` with no harness-internals
changes). It does **not** touch the PIXEL half — `gui-smoke/lib/compare.ts`'s screenshot-baseline
blessing is still open, unrelated mechanism, row #3 stays 🔧 rather than flipping ✅ until that lands
too._

_**2026-08-26 (CPE-1882 UAT correction, same shift):** the pass above verified the harness locally and
declared row #3's layout half done — it wasn't yet. **The `layout-guard` job crashed unconditionally on
every real CI run, before measuring a single pixel**: `.github/workflows/gui-smoke.yml` pinned it to
`node-version: 20` (copy-pasted from this workflow's other jobs), but `engine.mjs`'s CDP client calls
the global `WebSocket` constructor directly, which is only a stable Node built-in from v22 — on 20 it's
`ReferenceError: WebSocket is not defined`, every time, regardless of whether the branch under test has
a real layout regression. A gate that is red for the same reason on every run carries **zero**
information — worse than the flaky gate this ticket's own text warned against, because a flaky gate is
at least sometimes right. Root cause: every local red-proof in the pass above ran on local Node ≥22,
which never exercises what CI actually runs — exactly the gap between "the harness works" and "the
harness is wired into CI." Fixed by pinning just that one job to `node-version: 22` (kept at 20
everywhere else in this workflow — see that job's own comment for why). Lesson for future CI-job
tickets in this ledger: a claim that a CI job demonstrates something requires reading that job's own
real CI log, not a local run of the same script under a different Node version._

_**2026-08-26 (CPE-1882, CONFIRMED GREEN):** two more CI-only rounds after the Node fix above, both
found by reading real CI logs rather than trusting a local run: (1) the job's own 12-per-width fresh
Chrome-process launches hit its 10-minute timeout on a shared runner (fine on a dev workstation) —
fixed by reusing ONE Chrome instance for the whole sweep; (2) even after the sweep printed its verdict
in ~18s, the JOB then hung for the remaining ~14.7 minutes until an external timeout force-killed
orphaned `npm`/`vite`/`esbuild` processes — a POSIX `shell: true` spawn's immediate child (`sh`) dying
did not kill ITS OWN children, which kept this script's piped stdout/stderr open and stopped Node from
exiting naturally — fixed by killing the whole process group + a hard `process.exit()` backstop.
**Job `98387478392`: `pass`, 38s, GitHub's own check status** — confirmed via `gh pr checks`, not just
...
<!-- mvd-table: excluded — the 2026-07-26 "new manual debt from merged PRs" table. Its Status cells are
     ...
     Kept verbatim as history. Audited row-by-row against that criterion on 2026-08-27 (CPE-1946): the
     six rows below all pass it — their residual is feel/pixel/attended-observation and none has a
     retiring ticket. The one row that did NOT pass, CPE-1263, was moved out of this table into the
     supplementary ledger that same shift; see the CPE-1946 section at the foot. -->
...
_(One row of that excluded table, **CPE-1263**, was carried out of this shift as an open promote-or-close
question. It was **decided on 2026-08-27 by CPE-1946** — promoted into the supplementary ledger, taking
the total 13 → 14. See the CPE-1946 section at the foot for the argument. The **13** stated above is the
count as it stood at the end of THIS shift, kept as history per the header's rule.)_
...
## Promoted 2026-08-27 (CPE-1946) — CPE-1263 moves out of `excluded` and into the count

The 2026-07 tables are annotated `excluded` on one stated criterion: their residual is **feel/taste**,
which the CPE-1148 split reserves for the user — *“work that has no retiring ticket”*. CPE-1922 recorded
that one row did not meet it and deferred the call. Decided this shift, argued against the criterion
rather than against convenience: **promote.**

- **The residual is not feel/taste.** The row's own Status cell read *“logic automated (jsdom) —
...
- **The old “unburnable” premise is gone.** The obstacle that once justified treating this as unretirable
...
**Arithmetic:** primary 6 → 6 (unchanged), supplementary 7 → 8, total **13 → 14**. The count rises because
the ledger became more honest, not because anything regressed — the debt was always there, sitting in a
table that said it did not count. The **12** PR #1042's UAT tester reached independently was a check on
*the recount method*, and CPE-1922 spent it; it was never a ceiling on what these tables may contain.
...
deleted, because a recorded discrepancy is a good interim state and a bad permanent one.
