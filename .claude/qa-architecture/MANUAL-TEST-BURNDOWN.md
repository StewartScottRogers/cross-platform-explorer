...
| 3 | **Visual / theme regression** (light+dark, menus per MENUS.md, tabs per TABS.md, pill reflow) | none | ⛰ manual | Screenshot-diff harness over key screens in both themes; fail on unexpected pixel delta | CPE-1049 |
| 4 | **Cross-OS GUI** (macOS + Linux app behaviour, not just backend) | backend only (3-OS matrix) | ⛰ manual | macOS/Linux CI runners driving the headless GUI harness from #1 | CPE-1049 |
...
| 6 | **Auto-update flow** (updater downloads, verifies signature, swaps in place) | none end-to-end | ⛰ manual | Staged updater E2E against a test endpoint + signed test artifact in CI | CPE-1049 |
