...
unreachable — the exact pill/chip-reflow rule row #3 names, now enforced by CI rather than described in a
CLAUDE.md convention nobody re-checks. Red-proofed against both CPE-1836 (status bar git-block bleed) and
CPE-1827 (Trash titlebar clip at the 600px floor) — see the ticket's own Work Log for the full
before/after CI output. **This closes the LAYOUT half of row #3** (overlap/clip/text-escape, geometry-only,
2 components today, extensible via `cases.mjs` with no harness-internals changes). It does **not** touch
the PIXEL half — `gui-smoke/lib/compare.ts`'s screenshot-baseline blessing is still open, unrelated
mechanism, row #3 stays 🔧 rather than flipping ✅ until that lands too._
