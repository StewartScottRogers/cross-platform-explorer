---
id: CPE-198
title: Preview/edit support for Gradle build scripts files
type: Feature
status: Done
priority: Low
component: Frontend
...
Add a preview provider for Gradle build scripts (.gradle) in the right-side preview pane. Highlighted Gradle. Editable as source text, saved via the write_file_text command (CPE-066).

## Acceptance Criteria

- [ ] .gradle is matched by a dedicated preview provider in the bundled registry
...
Part of the [[CPE-059]] preview architecture. Approach: highlight.js groovy/kotlin. Editing model: source.
